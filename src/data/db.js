...
function getReport(lineUserId, lookback, interval) {
  if (intervalTypes.includes(interval) === false) {
    throw new Error('ช่วงเวลาไม่ถูกต้อง ระบุเป็น "days", "months", หรือ "years" ได้เท่านั้น');
  }

  const lookbackPreparedStr = `-${lookback} ${interval}`;

  const getTransactionCount = db.prepare(`select count(*) as transactionCount from transactions where lineUserId = ? and datetime(date) >= datetime('now', ?)`);
  const getTotalAmount = db.prepare(`select sum(amount) as totalAmount from transactions where lineUserId = ? and datetime(date) >= datetime('now', ?)`);

  const transactions = getTransactionCount.get(lineUserId, lookbackPreparedStr).transactionCount;
  const totalAmount = getTotalAmount.get(lineUserId, lookbackPreparedStr).totalAmount;
